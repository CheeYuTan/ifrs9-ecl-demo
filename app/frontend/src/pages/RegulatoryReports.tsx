          ...
          {REPORT_TYPES.map(rt => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-3 flex items-center gap-1 text-xs font-semibold text-slate-400 group-hover:text-brand transition-colors">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1 font-mono">{report.report_id}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {report.status === 'draft' && (
            <button onClick={() => onFinalize(report.report_id)}
              className="flex items-center gap-2 px-4 py-2 text-xs font-bold text-white bg-gradient-to-r from-purple-500 to-purple-600 rounded-xl shadow-lg hover:opacity-90 transition">
      ...
      <Card>
        <div className="grid grid-cols-4 gap-6 text-sm">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Report Date</p>
          ...
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Generated By</p>
            <p className="font-semibold text-slate-700 dark:text-slate-200">{report.generated_by}</p>
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Generated At</p>
            ...
            return (
              ...
              >
                ...
                {section?.data?.length > 0 && (
                  <span className={`ml-2 px-1.5 py-0.5 rounded-full text-[10px] ${isActive ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-700'}`}>
                    {section.data.length}
                  </span>
                )}
              </button>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={activeSection} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.15 }}>
          {currentSection ? (
            ...
            <Card>
              <p className="text-sm text-slate-400 text-center py-8">Select a section to view</p>
            </Card>
          )}
        </motion.div>
      </AnimatePresence>
