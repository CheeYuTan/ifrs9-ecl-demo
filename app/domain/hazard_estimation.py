...
from domain.hazard_cox_ph import _estimate_cox_ph, _build_segment_curves
from domain.hazard_nonparam import _estimate_discrete_time, _estimate_kaplan_meier

log = logging.getLogger(__name__)


def estimate_hazard_model(model_type: str, product_type: str | None = None,
    ...
    model_id = f"haz_{model_type}_{_dt.now(_tz.utc).strftime('%Y%m%d%H%M%S')}_{str(uuid.uuid4())[:8]}"
    ...
    """, (model_id, model_type,
          ...
          result["concordance_index"], result["log_likelihood"],
          result["aic"], result["bic"],
          product_type, segment, n_obs, n_events))
    ...
    for curve in curves:
        ...
        execute(f"""
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (curve_id, model_id, curve.get("segment", "all"),
