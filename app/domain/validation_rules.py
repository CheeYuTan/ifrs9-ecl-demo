...
class ValidationResult:
    ...
    def __init__(self, rule_id: str, category: str, severity: str,
                 passed: bool, message: str, detail: dict | None = None):
...
def check_scenario_weights_sum(weights: dict[str, float]) -> ValidationResult:
    ...
    return ValidationResult(
        "D1", "data_integrity", "CRITICAL", passed,
...
def check_pd_range(pd_values: list[float]) -> ValidationResult:
    ...
    return ValidationResult(
        "D2", "data_integrity", "CRITICAL", passed,
...
def check_lgd_range(lgd_values: list[float]) -> ValidationResult:
    ...
    passed = len(violations) == 0
    return ValidationResult(
        "D3", "data_integrity", "CRITICAL", passed,
...
def check_eir_positive(eir_values: list[float]) -> ValidationResult:
    ...
    passed = len(violations) == 0
    return ValidationResult(
        "D4", "data_integrity", "CRITICAL", passed,
...
def check_remaining_months(months: list[float]) -> ValidationResult:
    ...
    passed = len(violations) == 0
    return ValidationResult(
        "D5", "data_integrity", "CRITICAL", passed,
...
def check_gca_positive(gca_values: list[float]) -> ValidationResult:
    ...
    passed = len(violations) == 0
    return ValidationResult(
        "D6", "data_integrity", "CRITICAL", passed,
...
def check_origination_before_reporting(
    origination_dates: list[str], reporting_date: str
) -> ValidationResult:
    ...
    try:
        ...
        return ValidationResult(
            "D9", "data_integrity", "CRITICAL", False,
    ...
    passed = violations == 0
    return ValidationResult(
        "D9", "data_integrity", "CRITICAL", passed,
...
def check_maturity_after_origination(
    ...
    passed = violations == 0
    return ValidationResult(
        "D10", "data_integrity", "HIGH", passed,
...
def check_coverage_monotonic(stage_coverage: dict[int, float]) -> ValidationResult:
    ...
    passed = s1 <= s2 <= s3
    return ValidationResult(
        "M-R1", "model_reasonableness", "HIGH", passed,
...
def check_ecl_period_change(current_ecl: float, prior_ecl: float) -> ValidationResult:
    ...
    if prior_ecl == 0:
        return ValidationResult("M-R2", "model_reasonableness", "MEDIUM", True,
                                "No prior period ECL for comparison", {})
    ratio = current_ecl / prior_ecl
    passed = 0.5 <= ratio <= 2.0
    return ValidationResult(
        "M-R2", "model_reasonableness", "MEDIUM", passed,
...
def check_convergence_cv(cv: float, threshold: float = 0.05) -> ValidationResult:
    ...
    passed = cv < threshold
    return ValidationResult(
        "M-R4", "model_reasonableness", "HIGH", passed,
        f"Convergence CV: {cv:.4f} (threshold: {threshold})",
        {"cv": cv, "threshold": threshold},
    )


def check_scenario_concentration(scenario_ecl: dict[str, float],
                                 total_ecl: float) -> ValidationResult:
    """M-R5: No single scenario should contribute > 40% of probability-weighted ECL."""
    if total_ecl == 0:
        return ValidationResult("M-R5", "model_reasonableness", "MEDIUM", True,
                                "Total ECL is zero", {})
    max_pct = 0
    max_scenario = ""
...
def check_satellite_r_squared(r_squared_values: dict[str, float],
    ...
    return ValidationResult(
        "M-R3", "model_reasonableness", "HIGH", passed,
...
def check_pd_aging_factor(aging_factor: float,
                           min_val: float = 0.0,
                           max_val: float = 0.30) -> ValidationResult:
    """M-R7: PD aging factor must produce increasing marginal PD over time."""
    passed = min_val <= aging_factor <= max_val
    return ValidationResult(
        "M-R7", "model_reasonableness", "HIGH", passed,
...
def check_scenario_weight_constraints(weights: dict[str, float]) -> ValidationResult:
    ...
    passed = len(issues) == 0
    return ValidationResult(
        "M-R5b", "model_reasonableness", "MEDIUM", passed,
...
def check_segregation_of_duties(executor_user: str,
                                 signoff_user: str) -> ValidationResult:
    """G-R1: ECL run must not be signed off by the same user who executed it."""
    passed = executor_user != signoff_user
    return ValidationResult(
        "G-R1", "governance", "CRITICAL", passed,
...
def check_overlay_cap(total_overlay: float, base_ecl: float,
                      cap_pct: float = 15.0) -> ValidationResult:
    """M-R8: Total management overlays should not exceed cap% of base ECL."""
    if base_ecl == 0:
        return ValidationResult("M-R8", "model_reasonableness", "MEDIUM", True,
                                "Base ECL is zero", {})
    overlay_pct = abs(total_overlay) / abs(base_ecl) * 100
    passed = overlay_pct <= cap_pct
    return ValidationResult(
        "M-R8", "model_reasonableness", "MEDIUM", passed,
...
def check_ead_non_negative(ead_values: list[float]) -> ValidationResult:
    ...
    passed = len(violations) == 0
    return ValidationResult(
        "DA-1", "domain_accuracy", "CRITICAL", passed,
        f"EAD non-negative check: {len(violations)} negative values out of {len(ead_values)}",
        {"violations_count": len(violations), "total_values": len(ead_values),
         "ifrs_ref": "IFRS 9.B5.5.31"},
    )
...
def check_stage3_pd_consistency(
    ...
    passed = len(violations) == 0
    return ValidationResult(
        "DA-3", "domain_accuracy", "HIGH", passed,
        f"Stage 3 PD consistency: {len(violations)} of {len(s3_pairs)} Stage 3 loans have PD < {min_stage3_pd}",
        {"violations_count": len(violations), "total_stage3": len(s3_pairs),
         "min_stage3_pd": min_stage3_pd, "ifrs_ref": "IFRS 9.B5.5.37"},
    )
...
def check_discount_rate_valid(eir_values: list[float]) -> ValidationResult:
    ...
    passed = len(violations) == 0
    return ValidationResult(
        "DA-4", "domain_accuracy", "CRITICAL", passed,
        f"Discount rate validity: {len(violations)} EIR values <= -1 out of {len(eir_values)}",
        {"violations_count": len(violations), "total_values": len(eir_values),
         "ifrs_ref": "IFRS 9.B5.5.44"},
    )
...
def check_ecl_non_negative(ecl_values: list[float]) -> ValidationResult:
    ...
    passed = len(violations) == 0
    return ValidationResult(
        "DA-5", "domain_accuracy", "HIGH", passed,
...
def check_minimum_scenario_count(
    scenario_count: int, minimum: int = 3,
) -> ValidationResult:
    ...
    return ValidationResult(
        "DA-6", "domain_accuracy", "HIGH", passed,
        f"Scenario count: {scenario_count} (minimum required: {minimum})",
        {"scenario_count": scenario_count, "minimum": minimum,
         "ifrs_ref": "IFRS 9.B5.5.42"},
    )
