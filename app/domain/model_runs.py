...
def save_model_run(run_id: str, run_type: str, models_used: list, products: list,
                    total_cohorts: int, best_model_summary: dict, notes: str = "") -> dict:
    ...
    """, (run_id, run_type, _json.dumps(models_used), _json.dumps(products),
          total_cohorts, _json.dumps(best_model_summary), notes))
...
def _detect_available_dimensions(tbl: str) -> list[str]:
    ...
    DIMENSION_PRIORITY = [
        "credit_grade", "risk_band", "delinquency_bucket", "region",
        "age_bucket", "employment_type", "vintage_year", "ltv_band",
        "industry_sector", "assessed_stage", "segment",
    ]
    available = []
