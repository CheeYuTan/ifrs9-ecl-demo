...
def prepare_loan_columns(loans_df: pd.DataFrame, base_lgd: dict) -> pd.DataFrame:
    ...
    # Drop rows with critical nulls that would break the simulation
...
def run_scenario_sims(
    *, rng, n_loans, n_sims, batch_size,
    rho_1d, base_pd, base_lgd_arr,
    pd_mult, lgd_mult, pd_vol, lgd_vol,
    pd_floor, pd_cap, lgd_floor, lgd_cap,
    aging_factor, is_stage_23_1d,
    max_horizon, global_max_q,
    quarterly_prepay, rem_months_f, is_bullet, gca, eir,
    products, unique_products, product_sim_ecls, w,
):
