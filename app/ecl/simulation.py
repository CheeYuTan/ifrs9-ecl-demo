...
from ecl.helpers import _emit, _convergence_check_from_paths
...
def run_simulation(
    ...
    for i, sc in enumerate(scenarios):
        ...
        loan_ecl_accum, portfolio_path_ecls = run_scenario_sims(
            rng=rng, n_loans=n_loans, n_sims=n_sims, batch_size=BATCH_SIZE,
            rho_1d=product_corr, base_pd=base_pd, base_lgd_arr=base_lgd_arr,
            pd_mult=sc_data["pd_mult"], lgd_mult=sc_data["lgd_mult"],
            pd_vol=sc_data["pd_vol"], lgd_vol=sc_data["lgd_vol"],
            pd_floor=pd_floor, pd_cap=pd_cap, lgd_floor=lgd_floor, lgd_cap=lgd_cap,
            aging_factor=aging_factor, is_stage_23_1d=(stage != 1),
            max_horizon=max_horizon, global_max_q=global_max_q,
            quarterly_prepay=quarterly_prepay, rem_months_f=rem_months_f,
            is_bullet=is_bullet, gca=gca, eir=eir,
            products=products, unique_products=_unique_products,
            product_sim_ecls=product_sim_ecls, w=w,
        )
        ...
        _emit(progress_callback, {
            ...
            "detail": {
                ...
                "mean_pd_mult": sc_data["pd_mult"], "mean_lgd_mult": sc_data["lgd_mult"],
    ...
    return aggregate_results(
        loans_df=loans_df, loan_weighted_ecl=loan_weighted_ecl,
        scenario_ecl_totals=scenario_ecl_totals,
        scenario_ecl_paths=scenario_ecl_paths,
        product_sim_ecls=product_sim_ecls, products=products,
        scenarios=scenarios, weights=weights,
        n_sims=n_sims, n_loans=n_loans,
        sim_params={
            "n_sims": n_sims, "random_seed": random_seed,
            "pd_lgd_correlation": pd_lgd_correlation, "aging_factor": aging_factor,
            "pd_floor": pd_floor, "pd_cap": pd_cap,
            "lgd_floor": lgd_floor, "lgd_cap": lgd_cap,
        },
        t0=t0, load_time=load_time, progress_callback=progress_callback,
    )
