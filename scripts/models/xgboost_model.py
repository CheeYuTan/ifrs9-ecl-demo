"""
Satellite Model: XGBoost (Optuna TPE Tuning)
==============================================
Extreme Gradient Boosting with Optuna TPE sampler for efficient search
over n_estimators, max_depth, learning_rate, subsample, regularization.
Falls back to sklearn GradientBoosting if xgboost is not installed.
...
def fit(X, y):
    ...
    if use_xgb:
        best = XGBRegressor(**best_params, random_state=42, verbosity=0)
    else:
        from sklearn.ensemble import GradientBoostingRegressor
    ...
    return {
        ...
        "formula": f"XGB(Optuna TPE {N_TRIALS} trials+pruning, {params_str}) importances: {', '.join(f'{k}={v:.3f}' for k, v in importances.items())}",
